...
void ofApp::setup(){
    ...
    parameterLoader.setup("parameters.txt");
    ...
    attractions[0] = ofPoint(0.2, 0.2);
    attractions[1] = ofPoint(0.8, 0.2);
    attractions[2] = ofPoint(0.5, 0.8);
...
void ofApp::update(){
    ...
    for(int i = 0; i<3; i++){
        ...
        float time = ofGetElapsedTimef() * (i+3) *0.1;
        float range = abs(cos(ofGetElapsedTimef()*0.1 * (7-i))*0.4) ;
      //  attractions[i].x =MIN( sin(time) * range + 0.5 , 1.0 );
      //  attractions[i].y = MIN( cos(time) * range + 0.5 , 1.0 );
        
//        attractions[i].x +=ofRandom(-i*2.,i*2) * 0.001;
//        attractions[i].y +=ofRandom(-i*2.,i*2) * 0.001;
//
//        CLAMP(attractions[i].x,0.f,1.f);
//        CLAMP(attractions[i].y,0.f,1.f);
    }
...
//    float x = float(ofGetMouseX()) / float(width);
//    float y = float(ofGetMouseY()) /float(height);
    updateVel.setUniforms(parameterLoader.guiGroup);
    updateVel.setUniform2f("a1", attractions[0].x,attractions[0].y);
    updateVel.setUniform2f("a2", attractions[1].x,attractions[1].y);
    updateVel.setUniform2f("a3", attractions[2].x,attractions[2].y);
    updateVel.setUniformTexture("posData", posPingPong.src->getTexture(), 1);  // passing the position information

    updateVel.setUniform1f("timestep", (float)timeStep);
    updateVel.setUniform1f("time", (float)ofGetElapsedTimef());
    ...
    ofClear(0);
    updatePos.begin();
    updatePos.setUniforms(parameterLoader.guiGroup);
    ...
    ofClear(0,0,0,0);
    updateRender.begin();
    updateRender.setUniforms(parameterLoader.guiGroup);

    updateRender.setUniformTexture("posTex", posPingPong.src->getTexture(), 0);
    updateRender.setUniformTexture("velData", velPingPong.src->getTexture(), 1);   // passing the previus velocity
...
void ofApp::draw(){
    ...
    //ofDrawBitmapString("numParticles: " + ofToString( numParticles), 15,30);
    
    if(record){
    ofSetColor(255,200,200,100);
    for(int i = 0; i<3; i++){
        
        ofDrawCircle(attractions[i].x*width, attractions[i].y*height, 20);
 
        
    }
    }
    
    parameterLoader.draw();
}
...
void ofApp::mousePressed(int x, int y, int button){
   // attractions[setAttractions] = ofPoint( float(x)/float(width), float(y)/float(height) );
    cout << attractions[setAttractions] << endl;
}

//--------------------------------------------------------------
void ofApp::mouseReleased(int x, int y, int button){
    setAttractions++;
    setAttractions = setAttractions%3;
}
